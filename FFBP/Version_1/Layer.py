...
class Layer:
    def __init__(self, output_layer_flag, layer_length, layer_weight, layer_bias):
        ...
        self.error = 0
        self.output_layer_flag = 0
        self.layer_length = 0
        self.layer_weight = []
        self.layer_bias = []
        self.perceptrons = []
        self.__check_output_layer_flag(output_layer_flag)
        self.__check_layer_length(layer_length)
        self.__check_layer_weight(layer_weight)
        self.__check_layer_bias(layer_bias)
    ...
    def get_error_vector(self, desired_output):
        self.__check_desired_output(desired_output)
        try:
            if self.error == 1:
    ...
    def get_layer_output_vector(self, input):
        try:
            ...
            for i in range(self.layer_length):
                self.perceptrons[i].calc_activity(input)
    ...
    def calc_layer_delta_weights(self, eta):
        try:
            ...
            # set delta weights
            for i in range(self.layer_length):
                self.perceptrons[i].set_delta_weights(eta)
        except:
            self.error = 1
            raise
    ...
    def __check_output_layer_flag(self, output_layer_flag):
        ...
        self.error = 1
        try:
            if output_layer_flag == 0 or output_layer_flag == 1:
                self.error = 0
