##
## name: DLDataTankExample.py
## purpose: Example usage of DLDataTank
## date: 04/25/2019
##
##

import DLDataTank as dlt

if __name__ == '__main__':
	## Create DLDataTank object
	dtank = dlt.DLDataTank("../Data/Sumedh/DL_liver_32_32.csv")
	
	## Links to all of DL Dataset zips
	links = [
	...
	## Retrieve remote data files, store in data tank
	## Use links one at a time
	# dtank.load(links[1], "../Data/Images_png"

	## Retrieve Raghav's data files, store in data tank
	
	## Used csv to add File_name
	# raghav = pd.read_csv("../Data/Raghav/NNProject_liver_image_dataset_info.csv")
	# raghav["File_name"] = "../Data/Raghav/Images_png/"+raghav["File_name"]
	# dtank.retrieve(raghav["File_name"])
	
	## Used find `pwd` -name *.png > raghav_listOfFiles.txt to get image files in directory
	raghav = [
	...
	dtank.retrieve(raghav)
	print(len(dtank.getImages()))

	## Retrieve Sumedh's data files, store in data tank
	## Used find `pwd` -name *.png > sumedh_listOfFiles.txt to get image files in directory
	sumedh = [
	...
	dtank.retrieve(sumedh)
	print(len(dtank.getImages()))

	## Generate array of data desired
	test_data = dtank.generate(32, 32, 2)
	print(len(test_data))
